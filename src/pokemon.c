...
void cargar_ataque(pokemon_t *pokemon, char *linea) {
    ...
    if (sscanf(linea, "%49[^;];%c;%u\n", ataque->nombre, &variable_tipo, &ataque->poder) == 3) {
...
int con_cada_pokemon(informacion_pokemon_t *ip, void (*f)(pokemon_t *, void *), void *aux) {
    ...
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (strcmp(ip->pokemon[j]->nombre, ip->pokemon[j + 1]->nombre) > 0) {
...
pokemon_t *pokemon_buscar(informacion_pokemon_t *ip, const char *pokemon_que_quiero_encontrar) {
    ...
    for (int i = 0; i < ip->cantidad_de_pokemones; i++) {
        if (strcmp(ip->pokemon[i]->nombre, pokemon_que_quiero_encontrar) == 0) { 
            return ip->pokemon[i];
        }
    }
    return NULL; 
...
void crear_pokemon(informacion_pokemon_t *info, char *linea) {
   if (info == NULL || info->cantidad_de_pokemones >= MAX_POKEMONES) {
    ...
    if (sscanf(linea, "%19[^;];%c\n", variable_nombre, &variable_tipo) == 2) {
...
informacion_pokemon_t *pokemon_cargar_archivo(const char *path){
	...
	while (fgets(linea, sizeof(linea), archivo) != NULL) {
		...
		if (cantidad_delimitador == 1) {
			...
			cargar_ataque(info->pokemon[info->cantidad_de_pokemones - 1], linea);
		} else {
			break;
		}
	}
